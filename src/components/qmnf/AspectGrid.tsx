  ...
  return (
    <div className="rounded-lg border border-white/10 bg-black/30 p-4 backdrop-blur">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <select
          className="bg-black/40 border border-white/10 rounded text-xs font-mono px-2 py-1 text-white/70"
          value={filter}
        ...
        <table className="w-full text-xs font-mono">
          <thead className="text-[10px] uppercase tracking-widest text-white/40">
            <tr>
              ...
              {rigorous && <th className="text-right">c₇,c₁₁,c₁₃</th>}
            </tr>
          </thead>
          <tbody>
            {filtered.map((a, i) => (
              <tr key={i} className="border-t border-white/5">
                ...
                <td className="text-right text-white/60">
                  {(Number(a.orbDeltaArcsec) / 3600).toFixed(2)}°
                </td>
                {rigorous && <td className="text-right text-white/40">{a.carry.join(",")}</td>}
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr>
                <td colSpan={rigorous ? 4 : 3} className="py-4 text-center text-white/30">
                  no aspects in filter
                </td>
              </tr>
            )}
          </tbody>

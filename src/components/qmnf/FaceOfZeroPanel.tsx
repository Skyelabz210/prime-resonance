...
export function FaceOfZeroPanel({ chart }: { chart: FullChart }) {
  return (
    <div
      ...
      style={{
        borderColor: "#5dd6c433",
        background: "linear-gradient(135deg,#9d7bff10,#ffb34710)",
      }}
    >
      <div className="flex items-baseline justify-between mb-3">
        <h2 className="font-serif text-xl" style={{ color: "#e6e8ff" }}>
          Face of Zero
        </h2>
      ...
      <p className="text-xs text-white/50 mb-3 italic">
        Pairs locked on BOTH the Shadow lane and the Boundary lane. The chart's irreducible
        structural anchors.
      </p>
        ...
        {chart.faceOfZero.map((f, i) => (
          ...
          >
            ...
            <div className="text-[11px] font-mono mt-1">
              <span style={{ color: "#9d7bff" }}>
                Shadow: {SHADOW_LANE_NAMES[f.shadowResidue]} (r₁₁={f.shadowResidue})
              </span>
              {" · "}
              <span style={{ color: "#ffb347" }}>
                Boundary: {BOUNDARY_LANE_NAMES[f.boundaryResidue]} (r₁₃={f.boundaryResidue})
              </span>
            </div>
          </div>
        ))}

...
function geocentricLongitudeDeg(name: string, T: number): number {
  ...
  const lon = Math.atan2(dy, dx) / DEG;
  return norm360(lon);
}
...
// Dresden correction atlas — Swiss Ephemeris (DE440/441) − Meeus VSOP.
// Pure-integer linear interpolation between sample points.
// Outside the table range → returns 0 (graceful fallback to Meeus only).
import { DRESDEN_CORRECTIONS, DRESDEN_CORR_JD_START } from "./dresden_corrections.generated";

export function applyDresdenCorrection(jd: number, planet: string): bigint {
  const table = DRESDEN_CORRECTIONS[planet];
  if (!table) return 0n;
  const offset = jd - DRESDEN_CORR_JD_START;
  if (offset < 0) return 0n;
  const idxF = offset / table.step;
  const idx = Math.floor(idxF);
  if (idx + 1 >= table.data.length) return 0n;
  const c0 = table.data[idx];
  const c1 = table.data[idx + 1];
  const frac = idxF - idx;
  // Pure integer interpolation: c0 + ((c1 − c0) · stepFrac) / step, where
  // stepFrac is the float fractional part. JS Math.round keeps us integer.
  const v = c0 + Math.round((c1 - c0) * frac);
  return BigInt(v);
}
...
function rawLongitudeDeg(name: string, jd: number): number {
  ...
  switch (name) {
    case "Sun":
      return sunLongitudeDeg(T);
    case "Moon":
      return moonLongitudeDeg(T);
    case "NorthNode":
      return meanNodeDeg(T);
    case "Chiron":
      return chironLongitudeDeg(T);
    default:
      return geocentricLongitudeDeg(name, T);
  }
}

...
import {
  buildAspects, buildShadowNetwork, buildBoundaryNetwork,
  findFaceOfZero, findClassicallyInvisible, findAspectPatterns,
  type ClassifiedAspect, type ResidueBond, type FaceOfZero, type Pattern,
} from "./aspects";
...
export function computeFullChart(birth: BirthData): FullChart {
  const jd = datetimeToJD(
    ...
    birth.hour, birth.minute, 0, birth.tzOffsetHours,
  );
  ...
  return {
    birth, jd, ephemeris, houses,
    planetHouses, aspects, shadowNetwork, boundaryNetwork,
    faceOfZero, invisibleShadowBonds, patterns,
    maya: computeMaya(jd),
  };
}
...
export interface ReadingBundle {
  ...
  foundation: Array<{
    planet: string; position: string; retrograde: boolean; house: number;
    residues: { r2: string; r3: string; r5: string; r7: string; r11: string; r13: string };
    gearK: string;
    shadowLane: string; boundaryLane: string;
  }>;
...
function describePlanet(p: PlanetPosition, house: number) {
  ...
  return {
    ...
    house,
    residues: {
      r2: a.r2.toString(), r3: a.r3.toString(), r5: a.r5.toString(),
      r7: a.r7.toString(), r11: a.r11.toString(), r13: a.r13.toString(),
    },
...
export function buildReadingBundle(chart: FullChart): ReadingBundle {
  const b = chart.birth;
  const utcHour = b.hour - b.tzOffsetHours;
  const bornUTC = `${b.year}-${String(b.month).padStart(2,"0")}-${String(b.day).padStart(2,"0")} `
